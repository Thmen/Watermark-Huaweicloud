...
void write(data_20 buf[DCT_SIZE][DCT_SIZE], data_16 output[N])
{
   int r, c;
...
void add_watermark(data_8 indata[N], data_8 mark, data_8 outdata[N])
{
	...
	if(mark == 0)
	{
		buf_temp_qft[0][3] += 3600;
		//buf_temp_qft[1][2] += 10;
		buf_temp_qft[2][1] += 3600;
		//buf_temp_qft[3][0]+= 10;
	}
	else
	{
//		buf_temp_qft[0][3] += 10;
		buf_temp_qft[1][2] += 3680;
//		buf_temp_qft[2][1] += 10;
		buf_temp_qft[3][0]+= 3680;
	}
	//--------------------
...
void img_proc(data_8 img[IMG_SIZE*IMG_SIZE], data_8 mark[MARK_SIZE*MARK_SIZE], data_8 out_img[IMG_SIZE*IMG_SIZE])
{
...
PRELOAD_LOOP:
	for(mark_idx = 0; mark_idx < MARK_SIZE*MARK_SIZE; mark_idx++)
	{
		...
		out_img[base_idx]                   = outdata[0];
		out_img[base_idx + 1]               = outdata[1];
		out_img[base_idx + 2]               = outdata[2];
		out_img[base_idx + 3]               = outdata[3];
		out_img[base_idx + IMG_SIZE + 0]   = outdata[4];
		out_img[base_idx + IMG_SIZE + 1]   = outdata[5];
		out_img[base_idx + IMG_SIZE + 2]   = outdata[6];
		out_img[base_idx + IMG_SIZE + 3]   = outdata[7];
		out_img[base_idx + 2*IMG_SIZE + 0] = outdata[8];
		out_img[base_idx + 2*IMG_SIZE + 1] = outdata[9];
		out_img[base_idx + 2*IMG_SIZE + 2] = outdata[10];
		out_img[base_idx + 2*IMG_SIZE + 3] = outdata[11];
		out_img[base_idx + 3*IMG_SIZE + 0] = outdata[12];
		out_img[base_idx + 3*IMG_SIZE + 1] = outdata[13];
		out_img[base_idx + 3*IMG_SIZE + 2] = outdata[14];
		out_img[base_idx + 3*IMG_SIZE + 3] = outdata[15];
	}
}
