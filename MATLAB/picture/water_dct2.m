...
I=mark;
alpha=0.1;     %尺度因子,控制水印添加的强度,决定了频域系数被修改的幅度
k1=[1,0,1,0];  
k2=[0,1,0,1];
...
for i=1:rm          %在中频段嵌入水印
    for j=1:cm
        ...
        if mark(i,j)==1
            k=k1;
            after(x+1,y+1)=(floor(before(x+1,y+1)/50)-0.5)*50;
        else
            k=k2;
            after(x+1,y+1)=floor(before(x+1,y+1)/50)*50;
        end
%         beta = (before(x+1,y+4) + before(x+2,y+3) + before(x+3,y+2) + before(x+4,y+1))/alpha;
% %         beta = 5;
%         after(x+1,y+4)=before(x+1,y+4)+beta*k(1);
%         after(x+2,y+3)=before(x+2,y+3)+beta*k(2);
%         after(x+3,y+2)=before(x+3,y+2)+beta*k(3);
%         after(x+4,y+1)=before(x+4,y+1)+beta*k(4);
    end
end
